...
export default function Navbar({ text, logoutbutton }) {
    ...
    return (<nav className="w-full px-3 h-15 flex items-center justify-between  " >
        ...
        <h1 className="text-2xl font-bold text-orange-700" > Meja {text || 1}</h1>        
            <motion.button onClick={() => setshowprompt(true)} type="submit" className="flex items-center justify-center w-12 h-12 bg-orange-300 rounded-full " >
                <BoxArrowLeft size={26} className="text-orange-800" />
            </motion.button>   
            <Logoutprompt logoutbutton={logoutaction} showcondition={showprompt} cancelbutton={()=> setshowprompt(false)} />     
    </nav>)
}
